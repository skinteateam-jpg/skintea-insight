...
import { leadEvent, leadUpsert } from "@/lib/leads";
...
function QuizPage() {
  ...
  const next = () => {
    ...
    if (isLast) {
      ...
      // Lead capture: fields first, then the event (lead_event_add reads the stored lead).
      // The quiz collects no zip, age, budget, first-time flag or treatment ids.
      void (async () => {
        await leadUpsert({ skin_type: payload.skinType });
        await leadEvent("quiz_completed", { treatment_interest: payload.treatmentInterest });
      })();
