...
### 2026-09-18 07:50 UTC — Post updates timeline: updates_seen_at on saves, length checks, cleanup on post delete

- Who: pipeline session (Talk block 7), on Chi's spec (updatable timeline on Treatment and Surgery posts; the author
  alone adds; everyone who saved the post is told in the app; no email).
- What, one statement per call, each read back from `pg_catalog`:
  1. `saved_posts.updates_seen_at` and `surgery_saves.updates_seen_at`: `timestamptz NOT NULL DEFAULT now()`. An
     update newer than it is new to that saver; opening the post's page sets it to now. Existing saves got the time of
     the ALTER (there were 0 updates, so nothing was hidden). Both tables keep table-level grants, so no column grant.
  2. Policy `"Users mark their own saves seen"` on `surgery_saves`: UPDATE to authenticated, USING and WITH CHECK
     `auth.uid() = user_id`. `saved_posts` already had an own-row ALL policy.
  3. `post_updates_body_length` CHECK `char_length(btrim(body)) BETWEEN 1 AND 2000`; `post_updates_label_length`
     CHECK `label IS NULL OR char_length(btrim(label)) BETWEEN 1 AND 40`. The table held 0 rows.
  4. `public.delete_post_updates_on_post_delete()` (plpgsql, SECURITY DEFINER, `search_path = public`, EXECUTE
     revoked from PUBLIC, anon, authenticated) and AFTER DELETE triggers `posts_delete_updates`,
     `surgery_posts_delete_updates`, `product_posts_delete_updates`. `post_updates` has no foreign key (it serves three
     tables), so without this an author's updates outlived the post they deleted.
- Unchanged: `post_updates` policies (anyone reads; author inserts; author deletes), `enforce_post_update_author`, and
  the 07:05 revoke (`user_id` still unreadable by the client).
- Test, in a block that aborted: the author added a Treatment and a Surgery update; a blank body and a 41-character
  label were rejected; another member's update on that post was rejected ("only the author of the post may add an
  update to it"); that member saved both posts and updated `updates_seen_at` on both (1 row each) and saw 1 unseen
  update; anon read both updates and was denied `user_id`; deleting both posts left 0 updates. Afterwards 0 rows.
- App side (Lovable `92f42f8`…`3681a77`): `src/lib/postUpdates.ts`, `src/components/TalkUpdateTimeline.tsx`, new
  post pages `src/routes/treatment-talk_.$postId.tsx` and `src/routes/surgery-talk_.$postId.tsx` (the trailing
  underscore keeps them outside the feed layout), a link under each feed card with the update count and a "New" mark,
  and a "N new updates" pill on /skin-profile Saved Posts (`src/lib/userPosts.ts`, `src/routes/skin-profile.tsx`).
  `src/integrations/supabase/types.ts` was regenerated by Lovable (`d6a8f12`). `tsgo` and `bun run build` clean;
  the feeds, both post pages (missing-post state) and /tea render with no console errors and no response >= 400.
- Deleted session_ids: none. No rows inserted or deleted.
