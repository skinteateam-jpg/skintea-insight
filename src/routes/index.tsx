...
function Clinics({ clinics }: { clinics: any[] }) { return <section className="px-4 py-10 md:px-8"><div className="flex items-center gap-2"><MapPin size={19} className="text-brand-crimson" /><h2 className="text-xl font-extrabold">In Koreatown</h2></div><p className="mt-2 text-xs text-brand-muted">Ranked by real user data. Clinics can't buy position.</p><div className="mt-5 grid gap-3 md:grid-cols-3">{clinics.map((clinic) => <Link key={clinic.id} to="/clinics/$id" params={{ id: clinic.id }} className="rounded-md border border-brand-border bg-card p-5 text-foreground no-underline"><h3 className="font-extrabold">{clinic.name}</h3>{clinic.distanceMiles != null && <p className="mt-1 text-xs text-brand-muted">{Number(clinic.distanceMiles).toFixed(1)} miles</p>}<p className="mt-4 min-h-10 text-xs leading-5 text-brand-muted">{clinic.treatments.join(" · ") || "No treatments listed"}</p><div className="mt-4 border-t border-brand-border pt-4"><div className="flex min-h-5 items-center justify-between gap-3 text-xs"><span className="font-bold">Would go again</span><strong className={clinic.recommendPct == null ? "font-medium text-brand-muted" : "text-brand-crimson"}>{clinic.recommendPct == null ? "Not enough reviews yet" : `${clinic.recommendPct}%`}</strong></div><div className="mt-2 h-2 overflow-hidden rounded-full bg-secondary"><span className={`block h-full rounded-full ${clinic.recommendPct == null ? "bg-brand-muted/30" : "bg-brand-crimson"}`} style={{ width: `${clinic.recommendPct ?? 0}%` }} /></div></div></Link>)}</div><Link to="/clinics" className="mt-5 inline-flex items-center gap-2 text-sm font-bold text-brand-crimson no-underline">See all clinics in Koreatown <ArrowRight size={15} /></Link></section>; }
